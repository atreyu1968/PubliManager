...
const BooksManager: React.FC<Props> = ({ data, refreshData }) => {
  ...
  const closeModal = () => {
    ...
    setNewBook({ 
      ...
      status: 'Sin escribir', kindleUnlimited: false, scheduledDate: '', amazonLink: '', d2dLink: '', asin: '', driveFolderUrl: ''
    });
  };
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 bg-white p-6 rounded-[2rem] shadow-sm border border-slate-100">
        <div>
          ...
          <p className="text-[10px] text-slate-400 font-bold mt-1 uppercase tracking-widest">
            Vista: {isListView ? 'Lista' : 'Rejilla'} • Predeterminado: <span className="text-indigo-600">{data.settings.defaultLanguage}</span>
          </p>
        </div>
      ...
      <div className="space-y-4">
        ...
        <div className="flex flex-wrap gap-3 items-center">
          <div className="flex-1 min-w-[120px]">
            <select value={langFilter} onChange={e => setLangFilter(e.target.value)} className="w-full bg-white border border-slate-100 rounded-xl px-4 py-2.5 text-[10px] font-bold text-slate-600 outline-none">
              <option value="Todos">Idiomas: Todos</option>
              {LANGUAGES.map(l => <option key={l} value={l}>{l}</option>)}
            </select>
          </div>
          <div className="flex-1 min-w-[120px]">
            <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value as any)} className="w-full bg-white border border-slate-100 rounded-xl px-4 py-2.5 text-[10px] font-bold text-slate-600 outline-none">
              <option value="Todos">Estados: Todos</option>
              {STATUS_OPTIONS.map(opt => <option key={opt} value={opt}>{opt}</option>)}
            </select>
          </div>
          <div className="flex-1 min-w-[120px]">
            <select value={authorFilter} onChange={(e) => setAuthorFilter(e.target.value)} className="w-full bg-white border border-slate-100 rounded-xl px-4 py-2.5 text-[10px] font-bold text-slate-600 outline-none">
      ...
      <div className={isListView ? "space-y-3" : "grid grid-cols-1 xl:grid-cols-2 gap-4"}>
            ...
            <div key={book.id} className={`bg-white rounded-[2rem] border border-slate-100 shadow-sm hover:shadow-xl transition-all flex items-center gap-5 group relative overflow-hidden ${isListView ? 'p-3' : 'p-4'}`}>
              ...
              <div className="flex-1 min-w-0 py-1">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  {book.driveFolderUrl && (
                    <a href={book.driveFolderUrl} target="_blank" rel="noopener noreferrer" className="text-indigo-400 hover:text-indigo-600 transition-colors">
                ...
                <p className="text-[9px] font-bold text-slate-400 uppercase mt-1">{author?.name || 'Autor desconocido'}</p>
              </div>
        ...
        {filteredBooks.length === 0 && (
          <div className="col-span-full py-20 text-center bg-white rounded-[3rem] border border-dashed border-slate-200">
             <i className="fa-solid fa-book-open text-4xl text-slate-100 mb-4"></i>
             <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">No se encontraron libros para estos filtros</p>
          </div>
        )}
      </div>
            ...
            <div className="px-8 py-5 border-b border-slate-100 bg-white shrink-0 flex justify-between items-center">
              <div>
                <h2 className="text-xl font-black text-slate-900 uppercase tracking-tighter">
                  {editingId ? 'Editor Maestro' : 'Nuevo Proyecto Editorial'}
                </h2>
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                ...
                <div className="lg:col-span-4 space-y-6">
                  {!editingId && (
                    <div className="bg-slate-900 p-6 rounded-3xl space-y-4">
                      <label className="block text-[8px] font-black text-slate-500 uppercase tracking-widest">Modalidad de Lanzamiento</label>
                      <div className="grid grid-cols-1 gap-2">
                        <button 
                          onClick={() => setLaunchType('master')}
                          className={`flex items-center gap-3 p-3 rounded-xl border transition-all text-left ${launchType === 'master' ? 'bg-indigo-600 border-indigo-600 text-white' : 'bg-slate-800 border-slate-700 text-slate-400 hover:bg-slate-700'}`}
                        >
                          <i className="fa-solid fa-globe-americas"></i>
                          <div>
                            <p className="text-[9px] font-black uppercase tracking-widest">Maestro (7 Idiomas)</p>
                            <p className="text-[7px] font-bold opacity-60">Crea versiones para todo el ecosistema ASD</p>
                          </div>
                        </button>
                        <button 
                          onClick={() => setLaunchType('single')}
                          className={`flex items-center gap-3 p-3 rounded-xl border transition-all text-left ${launchType === 'single' ? 'bg-emerald-600 border-emerald-600 text-white' : 'bg-slate-800 border-slate-700 text-slate-400 hover:bg-slate-700'}`}
                        >
                          <i className="fa-solid fa-language"></i>
                          <div>
                            <p className="text-[9px] font-black uppercase tracking-widest">Individual ({data.settings.defaultLanguage})</p>
                            <p className="text-[7px] font-bold opacity-60">Solo para el idioma predeterminado</p>
                          </div>
                        </button>
                      </div>
                    </div>
                ...
                <div className="lg:col-span-8 space-y-6">
                  <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
                    <label className="block text-[9px] font-black text-slate-400 uppercase tracking-widest mb-3">Título Principal</label>
                  ...
                  <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
                    <label className="block text-[9px] font-black text-slate-400 uppercase mb-3">Carpeta Google Drive (Producción)</label>
                    <div className="relative">
                       <i className="fa-brands fa-google-drive absolute left-5 top-1/2 -translate-y-1/2 text-slate-400"></i>
                       <input 
                        placeholder="URL de la carpeta de trabajo (Manuscritos, diseños, etc.)" 
                        value={newBook.driveFolderUrl} 
                        onChange={e => setNewBook({...newBook, driveFolderUrl: e.target.value})}
                        className="w-full bg-slate-50 border border-slate-100 rounded-xl pl-14 pr-6 py-4 font-bold text-slate-600 focus:ring-4 focus:ring-indigo-500/10 outline-none transition-all shadow-inner text-xs"
                      />
                    </div>
                  </div>

                  <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
                    <label className="block text-[9px] font-black text-slate-400 uppercase mb-3">Sinopsis / Descripción</label>
