    ...
    <div className="space-y-8 animate-fadeIn">
          ...
          <tbody className="divide-y divide-slate-50">
            {data.sales.sort((a,b) => (b.year*12+b.month) - (a.year*12+a.month)).map(record => (
              <tr key={record.id} className="hover:bg-slate-50 transition">
                ...
                <td className="px-8 py-5 text-right font-mono text-xs">{record.units}</td>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-sm flex items-center justify-center z-[100] p-4">
          <div className="bg-white rounded-[2rem] p-8 max-w-lg w-full shadow-2xl animate-scaleIn">
            <h2 className="text-2xl font-black text-slate-900 mb-6 flex items-center gap-2">
              <i className="fa-solid fa-chart-line text-emerald-500"></i> Reporte Mensual
            </h2>
            <div className="space-y-4">
              <div>
                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Obra Seleccionada</label>
                <select value={newSale.bookId} onChange={e => setNewSale({...newSale, bookId: e.target.value})} className="w-full bg-slate-50 border-none rounded-xl p-4 text-sm font-bold">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Mes</label>
                  <select value={newSale.month} onChange={e => setNewSale({...newSale, month: parseInt(e.target.value)})} className="w-full bg-slate-50 border-none rounded-xl p-4 text-sm">
                ...
                <div>
                  ...
                  <input type="number" value={newSale.year} onChange={e => setNewSale({...newSale, year: parseInt(e.target.value)})} className="w-full bg-slate-50 border-none rounded-xl p-4 text-sm font-mono" />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Plataforma</label>
                  <select value={newSale.platform} onChange={e => setNewSale({...newSale, platform: e.target.value as any})} className="w-full bg-slate-50 border-none rounded-xl p-4 text-sm font-bold text-indigo-600">
                    <option value="KDP">Amazon KDP</option>
                    <option value="D2D">Draft2Digital</option>
                  </select>
                </div>
                <div>
                  <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Unidades Vendidas</label>
                  <input type="number" value={newSale.units} onChange={e => setNewSale({...newSale, units: parseInt(e.target.value) || 0})} className="w-full bg-slate-50 border-none rounded-xl p-4 text-sm font-mono" />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Páginas KENP (KDP)</label>
                  <input type="number" value={newSale.kenpc} onChange={e => setNewSale({...newSale, kenpc: parseInt(e.target.value) || 0})} className="w-full bg-slate-50 border-none rounded-xl p-4 text-sm font-mono" />
                </div>
                <div>
                  <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Ingresos (€)</label>
                  <input type="number" step="0.01" value={newSale.revenue} onChange={e => setNewSale({...newSale, revenue: parseFloat(e.target.value) || 0})} className="w-full bg-slate-50 border-none rounded-xl p-4 text-sm font-mono font-bold text-emerald-600" />
                </div>
              </div>
              <div className="flex gap-4 pt-6">
                ...
                <button onClick={handleAddSale} className="flex-1 py-4 bg-emerald-600 text-white rounded-xl font-black shadow-lg shadow-emerald-100 transition hover:scale-[1.02]">GUARDAR REPORTE</button>
              </div>
            </div>
          </div>
        </div>
